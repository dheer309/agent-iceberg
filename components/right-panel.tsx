...
import { X, ChevronDown, Pencil, RefreshCw, Trash2, GitBranchPlus, Ban, Loader2 } from 'lucide-react'
...
export function RightPanel({ projectId, nodeId, onClose }: RightPanelProps) {
  ...
  useEffect(() => {
    const fetchNodeDetails = async () => {
      try {
        const response = await fetch(`/api/project/${projectId}/node/${nodeId}`)
        if (response.ok) {
  ...
  const handleRegenerate = async () => {
    ...
    try {
      const response = await fetch(`/api/project/${projectId}/node/${nodeId}/modify`, {
        method: 'POST',
  ...
  return (
    <aside className="w-96 overflow-y-auto border-l border-border bg-card p-6">
        ...
        <CollapsibleContent className="mt-2 rounded-lg border border-border bg-muted/20 p-4">
          <div className="space-y-3 text-xs">
            ...
            {node.metadata.stepId && (
              <div>
                <div className="font-semibold text-foreground">Step ID</div>
                <div className="font-mono text-muted-foreground">{node.metadata.stepId}</div>
              </div>
            )}
            ...
            {node.metadata.toolInput && (
              <div>
                <div className="font-semibold text-foreground">Tool Input</div>
            ...
            {node.metadata.toolOutput && (
              <div>
                <div className="font-semibold text-foreground">Tool Output</div>
                <div className="font-mono text-muted-foreground whitespace-pre-wrap">
                  {node.metadata.toolOutput}
                </div>
              </div>
